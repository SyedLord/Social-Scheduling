        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl text-zinc-100 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="space-y-4">
              {/* Option 1: Instant Verified Sandbox Connect (Recommended for AI Studio instant testing) */}
              <div className="rounded-xl border border-blue-500/30 bg-blue-500/5 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-blue-400 uppercase tracking-wider">
                    Option 1: Instant 1-Click Connect
                  </span>
                ...
                <p className="text-xs text-zinc-300">
                  Instant sandbox linking with verified profile metadata, test OAuth tokens, and real dispatch capabilities.
                </p>

                <div>
                  <label className="block text-[11px] text-zinc-400 mb-1">
                    Custom Handle / Username (Optional)
                  </label>
                  <input
                    type="text"
                    placeholder={`e.g. @${selectedPlatform}_growth`}
                    value={customHandle}
                    onChange={(e) => setCustomHandle(e.target.value)}
                    className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-xs text-zinc-100 placeholder-zinc-500 focus:border-blue-500 focus:outline-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-2.5 px-4 text-xs font-semibold text-white hover:bg-blue-500 transition-colors shadow-md shadow-blue-600/20"
                >
                  <Sparkles className="h-3.5 w-3.5" />
                  <span>{loadingAction === 'connecting' ? 'Linking...' : 'Connect Channel Instantly'}</span>
                </button>
              </div>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4 space-y-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                    Option 2: Live OAuth 2.0 Redirect
                  </span>
                ...
                <p className="text-xs text-zinc-400">
                  Redirects to the official authorization screen. Requires configured Client IDs in `.env`.
                </p>
